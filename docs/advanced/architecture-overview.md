...
| `rakelib` | Contains individual ".rake" files that are helper scripts written in Ruby. These are used by Rake (set up by the "Rakefile" in the repository). Check the documentation in "docs/rake_tasks". |
...
| `Gemfile` | Gemfiles are used to manage Ruby dependencies. CB uses a basic Jekyll Gemfile plus some utilities need by our Rake tasks and plugins. |
| `Gemfile.lock` | The Gemfile.lock is generated by your computer to record the exact Ruby Gems used. By default CB gitignores the lock for simplicity. However, for more stability you may way to commit your lock to be able to reproduce the exact environment used to build your project. |
